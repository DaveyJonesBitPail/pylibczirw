...
/// found inside an IChannelDisplaySetting-object and tack on names and
...
struct ChannelDisplaySettingsStructWithNameAndDescription
  ...
  /// clears description. Note that name is not cleared.
  void Clear() {
...
/// Changes are accumulated in memory and only written to the file when Commit()
...
class PyCZIMetadataBuilder {
 ...
 public:
  ...
  /// Get the current XML representation of the metadata (for inspection or
  /// advanced editing). \param prettify If true, the XML is formatted with
  /// indentation. \returns The XML string representing the current state of
  /// the metadata.
  std::string GetXml(bool prettify = false) const;

  /// Replace the entire metadata XML content. For advanced users who need full
  /// control. \param xmlString The new XML content (must be valid CZI metadata
  /// XML with "ImageDocument" root). \throws LibCZIXmlParseException if the XML
  /// is malformed or missing required root node.
  ...
  /// Existing channels not in the map are preserved; channels in the map are
  /// updated/created. \param displaySettings Map of channel index -> channel
  /// display settings.
  ...
  void SetCustomKeyValue(const std::string &key,
...
class CZIeditAPI {
 ...
 public:
  ...
  /// The builder holds a copy of the metadata that can be modified
  /// independently. Call Commit() on the builder to write changes back to the
  /// file. \returns A shared pointer to the new metadata builder wrapper.
